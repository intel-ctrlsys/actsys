...
def creating_functions():
    ...
    op.execute(textwrap.dedent("""
        ...
        BEGIN
            IF (p_device_name is not null) THEN
                ...
                DELETE FROM public.sku_history WHERE device_id=v_device_id;
                DELETE FROM public.sku WHERE device_id=v_device_id;
    ...
    op.execute(textwrap.dedent("""
        CREATE OR REPLACE FUNCTION public.delete_sku( p_device_name character varying, p_sku_name character varying)
        RETURNS integer AS
        $BODY$
        DECLARE num_rows integer;
        BEGIN
            DELETE FROM public.sku WHERE sku.sku_name = p_sku_name
                AND sku.device_id IN (
                SELECT device_id FROM device WHERE device_type = p_device_name OR hostname = p_device_name
    ...
    op.execute(textwrap.dedent("""
        CREATE OR REPLACE FUNCTION public.get_sku_for_device(IN p_device_name character varying)
        RETURNS TABLE(device_id integer, sku_name character varying, step character varying, hardware_type character varying, model_number integer) AS
        $BODY$
        DECLARE
        v_device_id integer;
        BEGIN
            -- Get the device details
            IF (p_device_name is null) THEN
                RETURN QUERY SELECT device.device_id, sku.sku_name, sku.step, sku.hardware_type, sku.model_number
                FROM device
                INNER JOIN sku ON device.device_id = sku.device_id;
            ELSE
                v_device_id := public.get_device_id(p_device_name);
                RETURN QUERY SELECT device.device_id, sku.sku_name, sku.step, sku.hardware_type, sku.model_number
                FROM device
    ...
    op.execute(textwrap.dedent("""
        CREATE OR REPLACE FUNCTION public.get_sku_history_device(IN p_device_name character varying)
        RETURNS TABLE(hostname character varying, ip_address character varying, device_type character varying,
            step character varying, hardware_type character varying, model_number integer, sys_period tstzrange) AS
        $BODY$
        ...
        BEGIN
            ...
            IF (p_device_name is null) THEN
                RETURN QUERY
                SELECT device.hostname, device.ip_address, device.device_type, sku_history.step, sku_history.hardware_type,
                    sku_history.model_number , sku_history.sys_period
                FROM device
                INNER JOIN sku_history ON device.device_id = sku_history.device_id;
            ELSE
                RETURN QUERY
                SELECT device.hostname, device.ip_address, device.device_type, sku_history.step, sku_history.hardware_type,
                    sku_history.model_number, sku_history.sys_period
                FROM device
                INNER JOIN sku_history ON device.device_id = sku_history.device_id
                WHERE device.hostname = p_device_name OR device.ip_address = p_device_name OR device.device_type = p_device_name ;
            END IF;
            RETURN;
        END
        $BODY$
    ...
    op.execute(textwrap.dedent("""
        CREATE OR REPLACE FUNCTION public.upsert_sku(p_device_name character varying, p_sku_name character varying,
            p_step character varying, p_hardware_type character varying, p_model_number integer)
        RETURNS integer AS
        $BODY$
        ...
        BEGIN
            ...
            INSERT INTO public.sku(device_id, sku_name, step, hardware_type, model_number)
            VALUES (v_device_id, p_sku_name, p_step, p_hardware_type, p_model_number)
            ON CONFLICT (device_id, sku_name) DO UPDATE
            SET
                step = COALESCE(p_step, sku.step),
                hardware_type = COALESCE(p_hardware_type, sku.hardware_type),
                model_number = COALESCE(p_model_number, sku.model_number)
            WHERE sku.device_id = v_device_id AND sku.sku_name = p_sku_name;
...
def drop_functions():
    ...
    """))
    op.execute(textwrap.dedent("""
        DROP FUNCTION public.delete_sku(character varying, character varying);
    """))
    op.execute(textwrap.dedent("""
    ...
    """))
    op.execute(textwrap.dedent("""
        DROP FUNCTION public.get_sku_for_device(character varying);
    """))
    op.execute(textwrap.dedent("""
        DROP FUNCTION public.get_sku_history_device(character varying);
    """))
    op.execute(textwrap.dedent("""
...
def upgrade():
    ...
    op.create_table('sku_history',
                    ...
                    sa.Column('sku_name', sa.String(length=256), nullable=False),
