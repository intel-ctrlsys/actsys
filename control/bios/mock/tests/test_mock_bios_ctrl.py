...
class TestBiosCtrlMock(unittest.TestCase):
    ...
    def test_persist_state(self):
        ...
        self.assertTrue('No image found' in mock_nc.get_version('localhost'))
        self.assertTrue('Bios for' in mock_nc.bios_update('localhost', 'test.bin'))
        self.assertTrue('test.bin' in mock_nc.get_version('localhost'))

        mock_nc = MockNC(args=None)
        self.assertTrue(mock_nc.get_version('localhost'))
