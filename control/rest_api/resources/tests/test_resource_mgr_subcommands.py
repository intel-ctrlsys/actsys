...
from ..resource_mgr import ResourceManagerException, get_nodes_status
...
class TestResourceManagerRemove(TestResourceManagerBase):
    ...
    def test_create_put_remove_response_none_dictionaries(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_remove_response(None, None)
        self._check_exception(cmgr.exception, 409, 'Could not parse command results.', None, False)

    def test_create_put_remove_response_empty_dictionaries(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_remove_response({}, {})
        self._check_exception(cmgr.exception, 409, 'Could not parse command results.', None, False)

    def test_create_put_remove_response_single_success(self):
        self.cmd_invoker.resource_check.return_value = self.result['drain']
        success = create_dict_from_result(self.result['success'])
        ret = self.rmgr._create_put_remove_response(success, {})
        expected = {self.result['success'].device_name:dict(return_code=200, status='drain')}
        self.assertEqual(expected, ret)

    def test_create_put_remove_response_multiple_success(self):
        ...
        dict_successes = {self.result['success'].device_name:[self.result['success'],
                                                              self.result['success']]}
        ret = self.rmgr._create_put_remove_response(dict_successes, {})
        expected = {self.result['success'].device_name:dict(return_code=200, status='drain')}
        self.assertEqual(expected, ret)

    def test_create_put_remove_response_failure_bad_param_01(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_remove_response({}, create_dict_from_result(self.result['default']))
    ...
    def test_create_put_remove_response_failure(self):
        self.cmd_invoker.resource_check.return_value = self.result['alloc']
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_remove_response({}, create_dict_from_result(self.result['device']))
    ...
    def test_create_put_remove_response_multiple_failures(self):
        ...
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_remove_response({}, dict_failures)
        expected = {self.result['device'].device_name:dict(return_code=404, status='alloc'),
                    self.result['bad_param'].device_name:dict(return_code=400, status='invalid')}
        self._check_exception(cmgr.exception, 404, 'Could not remove node(s).', expected)

    def test_create_put_remove_response_multiple_failures_same_node(self):
    ...
    def test_create_put_remove_response_mixed(self):
        self.cmd_invoker.resource_check.return_value = [self.result['alloc'], self.result['drain']]
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_remove_response(create_dict_from_result(self.result['success']),
...
class TestResourceManagerCheck(TestResourceManagerBase):
    ...
    def test_create_put_check_response_none(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_check_response(None)
        self._check_exception(cmgr.exception, 409, 'Could not parse command results.')

    def test_create_put_check_response_single(self):
        ret = self.rmgr._create_put_check_response(self.result['idle'])
        expected = {self.result['idle'].device_name:self.result['idle'].message}
        self.assertEqual(expected, ret)

    def test_create_put_check_response_single_failed(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_check_response(self.result['device'])
    ...
    def test_create_put_check_response_multiple(self):
        ret = self.rmgr._create_put_check_response([self.result['idle'], self.result['drain']])
        expected = {self.result['idle'].device_name:self.result['idle'].message,
                    self.result['drain'].device_name:self.result['drain'].message}
        self.assertEqual(expected, ret)

    def test_create_put_check_response_multiple_failed(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_check_response([self.result['device'], self.result['default']])
        expected = {self.result['device'].device_name:'invalid',
                    self.result['default'].device_name:'invalid'}
        self._check_exception(cmgr.exception, 404, 'Could not get node(s) status.', expected, False)

    def test_create_put_check_response_multiple_mix(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_check_response([self.result['drain'], self.result['default']])
...
class TestResourceManagerAdd(TestResourceManagerBase):
    ...
    def test_create_put_add_response_none_successes(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response(None, {})
        self._check_exception(cmgr.exception, 409, 'Could not parse command results.')

    def test_create_put_add_response_none_failures(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response({}, None)
        self._check_exception(cmgr.exception, 409, 'Could not parse command results.')

    def test_create_put_add_response_single_success(self):
        self.cmd_invoker.resource_check.return_value = self.result['idle']
        success = create_dict_from_result(self.result['success'])
        ret = self.rmgr._create_put_add_response(success, {})
        expected = {self.result['success'].device_name:dict(return_code=200, status='idle')}
        self.assertEqual(expected, ret)

    def test_create_put_add_response_multiple_success(self):
        self.cmd_invoker.resource_check.return_value = [self.result['idle'], self.result['idle']]
        dict_successes = {self.result['success'].device_name:[self.result['success'],
                                                              self.result['success']]}
        ret = self.rmgr._create_put_add_response(dict_successes, {})
        expected = {self.result['success'].device_name:dict(return_code=200, status='idle')}
        self.assertEqual(expected, ret)

    def test_create_put_add_response_failure_bad_param_01(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response({}, create_dict_from_result(self.result['default']))
        expected = {self.result['default'].device_name:dict(return_code=400, status='invalid')}
        self._check_exception(cmgr.exception, 400, 'Could not add node(s).', expected)

    def test_create_put_add_response_failure_bad_param_02(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response({}, create_dict_from_result(self.result['bad_param']))
        expected = {self.result['bad_param'].device_name:dict(return_code=400,
                                                              status='invalid',
                                                              error='Invalid node_regex.')}
        self._check_exception(cmgr.exception, 400, 'Could not add node(s).', expected)

    def test_create_put_add_response_failure(self):
        self.cmd_invoker.resource_check.return_value = self.result['alloc']
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response({}, create_dict_from_result(self.result['device']))
        expected = {self.result['device'].device_name:dict(return_code=404, status='alloc')}
        self._check_exception(cmgr.exception, 404, 'Could not add node(s).', expected)

    def test_create_put_add_response_multiple_failures(self):
        dict_failures = deepcopy(create_dict_from_result(self.result['device']))
        dict_failures.update(create_dict_from_result(self.result['bad_param']))
        self.cmd_invoker.resource_check.return_value = [self.result['alloc'], self.result['default']]
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response({}, dict_failures)
        expected = {self.result['device'].device_name:dict(return_code=404, status='alloc'),
                    self.result['bad_param'].device_name:dict(return_code=400, status='invalid')}
        self._check_exception(cmgr.exception, 404, 'Could not add node(s).', expected)

    def test_create_put_add_response_multiple_failures_same_node(self):
        dict_failures = {self.result['device'].device_name:[self.result['device'],
                                                            self.result['device']]}
        dict_failures[self.result['device'].device_name][1].return_code = -1
        with self.assertRaises(ResourceManagerException) as cmgr:
    ...
    def test_create_put_add_response_mixed(self):
        self.cmd_invoker.resource_check.return_value = [self.result['alloc'], self.result['idle']]
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response(create_dict_from_result(self.result['success']),
                                               create_dict_from_result(self.result['device']))
        expected = {self.result['device'].device_name:dict(return_code=404, status='alloc'),
    ...
    def test_create_put_add_plugin_not_installed(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            self.rmgr._create_put_add_response({}, \
                create_dict_from_result(self.result['plugin_not_installed']))
        self._check_exception(cmgr.exception, 424, 'Resource Manager plugin is not installed')
