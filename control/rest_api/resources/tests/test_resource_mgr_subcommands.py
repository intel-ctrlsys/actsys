...
from ..resource_mgr import ResourceManagerException, get_nodes_status, _create_http_check_response
...
class TestResourceManagerCheck(TestResourceManagerBase):
    ...
    def test_create_http_check_response_multiple(self):
        ret = _create_http_check_response([self.result['idle'], self.result['drain']])
        expected = {self.result['idle'].device_name:self.result['idle'].message,
    ...
    def test_create_http_check_response_multiple_failed(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            _create_http_check_response([self.result['device'], self.result['default']])
        expected = {self.result['device'].device_name:'invalid',
    ...
    def test_create_http_check_response_multiple_mix(self):
        with self.assertRaises(ResourceManagerException) as cmgr:
            _create_http_check_response([self.result['drain'], self.result['default']])
