...
class TestOobSensorGetCommand(TestOobSensorCommand):
    ...
    def test_ret_msg(self):
        try:
            ...
            pass
        self.assertEqual(self.oob_sensor_get_all.execute().return_code, 0)
        self.assertEqual(self.oob_sensor_get1.execute().return_code, 0)
        self.assertEqual(self.oob_sensor_get.execute().return_code, 0)
        self.oob_manager_mock.get_sensor_value.return_value = {u'temp': [0.88765444]}
        self.assertEqual(self.oob_sensor_get.execute().return_code, 0)
